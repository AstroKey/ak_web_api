const DeviceType = require('./device_type.model')
...
module.exports.create = (req, res, next) => {
    return new DeviceType(req.body).save().then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.show = (req, res, next) => {
    return DeviceType.findById(req.params.id).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.update = (req, res, next) => {
    return DeviceType.findByIdAndUpdate(req.params.id, {$set: req.body}, { new: true }).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.delete = (req, res, next) => {
    return DeviceType.remove({_id: req.params.id }).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
