const DeviceTypeVersion = require('./device_type_version.model')
...
module.exports.create = (req, res, next) => {
    return new DeviceTypeVersion(req.body).save().then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.show = (req, res, next) => {
    return DeviceTypeVersion.findById(req.params.id).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.update = (req, res, next) => {
    return DeviceTypeVersion.findByIdAndUpdate(req.params.id, {$set: req.body}, { new: true }).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
...
module.exports.delete = (req, res, next) => {
    return DeviceTypeVersion.remove({_id: req.params.id }).then(function(response) {
        return res.status(200).send(response).end();
    }).catch(next);
};
